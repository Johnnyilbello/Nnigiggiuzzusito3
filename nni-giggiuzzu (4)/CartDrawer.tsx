...
import { X, Trash2, MessageCircle, ShoppingBag, MapPin, User, Bike, Store, AlertCircle, StickyNote, MinusCircle, Info, Instagram, Check, Plus, Truck, Clock, PlusCircle } from 'lucide-react';
...
const CartDrawer: React.FC<CartDrawerProps> = ({ 
  ...
  // Prezzo base items + Prezzo extra
  const subTotal = cartItems.reduce((sum, item) => {
      const extraTotal = item.selectedExtras ? item.selectedExtras.reduce((exSum, ex) => exSum + ex.price, 0) : 0;
  ...
  const handleWhatsAppOrder = () => {
    ...
    cartItems.forEach(item => {
      ...
      message += `▫️ ${item.quantity}x ${item.name} (€${(unitPrice * item.quantity).toFixed(2)})\n`;
      
      if (item.selectedVariant) {
          message += `   📏 *Var:* ${item.selectedVariant}\n`;
      }

      if (item.selectedExtras && item.selectedExtras.length > 0) {
          message += `   ➕ *Extra:* ${item.selectedExtras.map(e => e.name).join(', ')}\n`;
      }
                ...
                <div key={`${item.id}-${index}`} className="bg-[#1E1E1E] p-3 rounded-lg border border-gray-800 shadow-sm relative group">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1 pr-3">
                        ...
                        {/* Calcolo Prezzo Unitario per visualizzazione corretta (Base + Extra) */}
                        <p className="text-brand-orange font-mono text-sm">
                            €{((item.price + (item.selectedExtras ? item.selectedExtras.reduce((s,e) => s+e.price,0) : 0))).toFixed(2)}
                        </p>
                      </div>
                      <button 
                    ...
                    {/* Modifiche Automatiche (Extra Aggiunti) */}
                    {item.selectedExtras && item.selectedExtras.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-2">
                            {item.selectedExtras.map((ex, i) => (
                                <span key={i} className="inline-flex items-center gap-1 text-[10px] font-bold text-green-400 bg-green-900/20 px-1.5 py-0.5 rounded border border-green-900/30">
                                    <PlusCircle size={10} /> {ex.name}
                                </span>
                            ))}
                        </div>
                    )}
